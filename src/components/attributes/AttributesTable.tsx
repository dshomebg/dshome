...
export default function AttributesTable({ initialAttributes }: AttributesTableProps) {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      alert(error instanceof Error ? error.message : 'Грешка при изтриване на атрибут');
    } finally {
      setIsDeleting(null);
    }
  };
  ...
  const formatValues = (values: AttributeValue[] | undefined) => {
              ...
              attributes.map((attr) => (
                ...
                >
                  <td className="py-4 px-4">
                    <p className="text-black dark:text-white">{attr.id}</p>
                  </td>
                  <td className="py-4 px-4">
                    <p className="text-black dark:text-white font-medium">{attr.name}</p>
                  </td>
                  <td className="py-4 px-4">
                  ...
                  <td className="py-4 px-4">
                    <p className="text-sm text-gray-600 dark:text-gray-400 truncate max-w-xs">
                      {formatValues(attr.values)}
                    </p>
                  </td>
                  <td className="py-4 px-4">
                    <p className="text-black dark:text-white">{attr.position || 0}</p>
                  </td>
                  <td className="py-4 px-4">
                    <span
                    ...
                    >
                      {attr.isActive ? 'Активен' : 'Неактивен'}
                    </span>
                  </td>
                  <td className="py-4 px-4">
                    <div className="flex items-center space-x-3.5">
                      <Link
                        href={`/admin/attributes/edit/${attr.id}`}
