...
export default function SuppliersTable({ initialSuppliers }: SuppliersTableProps) {
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Сигурни ли сте, че искате да изтриете този доставчик?')) {
      return;
    }
    ...
    try {
      ...
      alert(error instanceof Error ? error.message : 'Грешка при изтриване на доставчик');
    } finally {
      setIsDeleting(null);
    }
  };
              ...
              suppliers.map((supplier) => (
                ...
                >
                  <td className="py-4 px-4">
                    <p className="text-black dark:text-white">{supplier.id}</p>
                  </td>
                  <td className="py-4 px-4">
                    <p className="text-black dark:text-white font-medium">{supplier.name}</p>
                  </td>
                  <td className="py-4 px-4">
