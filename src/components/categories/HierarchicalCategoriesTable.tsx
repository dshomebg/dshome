...
interface HierarchicalCategoriesTableProps {
  initialCategories: Category[];
}

export default function HierarchicalCategoriesTable({ initialCategories }: HierarchicalCategoriesTableProps) {
  ...
  // Групиране на категориите по родител
  ...
  // Рекурсивно рендиране на категории
  const renderCategory = (category: Category, level: number = 0) => {
    const children = getChildCategories(category.id);
    const isExpanded = expandedCategories.has(category.id);
    const hasChilds = hasChildren(category.id);

    return (
      <>
        <tr key={category.id} className="border-b border-stroke dark:border-strokedark hover:bg-gray-50 dark:hover:bg-gray-800">
          ...
          <td className="py-4 px-4">
            <div className="flex items-center gap-2" style={{ paddingLeft: `${level * 24}px` }}>
              ...
              {!hasChilds && <span className="w-4"></span>}
              <p className="text-black dark:text-white font-medium">{category.name}</p>
            </div>
          </td>
          <td className="py-4 px-4">
  ...
  // Взимаме само категориите от първо ниво (без родител)
    ...
    <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
      <div className="py-6 px-4 md:px-6 xl:px-7.5">
        ...
        <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
          Кликнете върху стрелката за да разгънете подкатегориите
        </p>
      </div>
          ...
          <tbody>
            {topLevelCategories.length === 0 ? (
              ...
              topLevelCategories.map(category => (
                <React.Fragment key={`top-${category.id}`}>
