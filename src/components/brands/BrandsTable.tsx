...
export default function BrandsTable({ initialBrands }: BrandsTableProps) {
  const [brands, setBrands] = useState<Brand[]>(initialBrands);
  const [isDeleting, setIsDeleting] = useState<number | null>(null);

  const handleDelete = async (id: number) => {
    if (!confirm('Сигурни ли сте, че искате да изтриете тази марка?')) {
      return;
    }
    ...
    try {
      ...
      alert(error instanceof Error ? error.message : 'Грешка при изтриване на марка');
    } finally {
      setIsDeleting(null);
    }
  };
              ...
              brands.map((brand) => (
                ...
                >
                  <td className="py-4 px-4">
                    <p className="text-black dark:text-white">{brand.id}</p>
                  </td>
                  <td className="py-4 px-4">
                    {brand.logoUrl ? (
                      ...
                      <div className="w-16 h-16 bg-gray-200 dark:bg-gray-700 rounded flex items-center justify-center">
                        <span className="text-gray-400 text-xs">Без лого</span>
                      </div>
                    )}
                  </td>
                  <td className="py-4 px-4">
                    <p className="text-black dark:text-white font-medium">{brand.name}</p>
                  </td>
                  <td className="py-4 px-4">
