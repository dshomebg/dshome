...
export default function FeaturesTable({ initialFeatures }: FeaturesTableProps) {
  const [features, setFeatures] = useState<Feature[]>(initialFeatures);
  const [isDeleting, setIsDeleting] = useState<number | null>(null);

  const handleDelete = async (id: number) => {
    if (!confirm('Сигурни ли сте, че искате да изтриете тази характеристика?')) {
      return;
    }
    ...
    try {
      ...
      alert(error instanceof Error ? error.message : 'Грешка при изтриване на характеристика');
    } finally {
      setIsDeleting(null);
    }
  };
              ...
              features.map((feature) => (
                ...
                >
                  <td className="py-4 px-4">
                    <p className="text-black dark:text-white">{feature.id}</p>
                  </td>
                  <td className="py-4 px-4">
                    <p className="text-black dark:text-white font-medium">{feature.name}</p>
                  </td>
                  <td className="py-4 px-4">
                    <p className="text-black dark:text-white">{feature.position || 0}</p>
                  </td>
                  <td className="py-4 px-4">
                    <span
                    ...
                    >
                      {feature.isActive ? 'Активна' : 'Неактивна'}
                    </span>
                  </td>
                  <td className="py-4 px-4">
